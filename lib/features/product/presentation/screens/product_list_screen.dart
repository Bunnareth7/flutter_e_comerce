//import 'package:e_com_app/features/search/presentation/bloc/search_history_bloc.dart';
import 'package:e_com_app/features/search/presentation/bloc/search_history_bloc.dart.dart';
//import 'package:e_com_app/features/search/presentation/bloc/search_history_event.dart';
import 'package:e_com_app/features/search/presentation/bloc/search_history_event.dart.dart';
//import 'package:e_com_app/features/search/presentation/bloc/search_history_state.dart';
...
class _ProductListScreenState extends State<ProductListScreen> {
                      ...
                      child: TextField(
                        ...
                        onSubmitted: (value) {
                          if (value.trim().isNotEmpty) {
                            context.read<SearchHistoryBloc>().add(AddSearchQuery(value.trim()));
                          }
                        },
                  ...
                  builder: (context, state) {
                    if (state is SearchHistoryLoaded && state.history.isNotEmpty) {
                      return Padding(
                        ...
                        child: Wrap(
                          spacing: 8,
                          children: state.history.map((item) => ActionChip(
                            label: Text(item.query),
                            onPressed: () {
                              _searchController.text = item.query;
                              setState(() {
                                _searchQuery = item.query.toLowerCase();
                                _showHistory = false;
                              });
                              context.read<SearchHistoryBloc>().add(AddSearchQuery(item.query));
                            },
                          )).toList(),
                        ),
                      );
  ...
  Widget _buildProductCard(product) {
    ...
    final discountedPrice = hasDiscount ? product.price * (1 - product.discountPercent! / 100) : product.price;
                  ...
                  if (hasDiscount)
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: Text('${product.discountPercent!.round()}% OFF',
